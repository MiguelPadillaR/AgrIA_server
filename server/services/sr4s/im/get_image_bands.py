...
from .utils import *
from ....config.constants import BANDS_DIR, SIZE
from ..constants import DELTA_DAYS
...
def download_image_bands(lat, lon, size, zoom, bands=None):
    ...
    for band in bands:
        ...
        download_sentinel_image(lat, lon, size, filename, evalscript_band)
        print()

def download_sentinel_bands(lat, lon):
    """
    ...
    size = (SIZE,SIZE)
    download_image_bands(lat, lon, size, bands)
